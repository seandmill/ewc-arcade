...
const WeatherEffects = ({ weather }: { weather: WeatherType }) => {
  ...
  // Set up Fog based on weather
  useEffect(() => {
    let fogColor = new THREE.Color('#cceeff');
    ...
    if (weather === WeatherType.Fog) {
      ...
      fogColor = new THREE.Color('#e2e8f0');
      fogDensity = 0.04;
    }
  ...
  useFrame((state) => {
    if ((weather === WeatherType.Rain || weather === WeatherType.Snow) && particlesRef.current) {
        const time = state.clock.elapsedTime;
        
        for(let i=0; i<particleCount; i++) {
             // Deterministic random start positions to avoid full re-calc
             const x = (i % 100) / 100 * GRID_SIZE * 3 - GRID_SIZE * 1.5;
             const z = Math.floor(i / 100) / 20 * GRID_SIZE * 3 - GRID_SIZE * 1.5;
             
             // Falling logic
             let ySpeed = weather === WeatherType.Rain ? 15 : 2;
             let yOffset = (time * ySpeed + i * 0.1) % 20; // 0 to 20
             let y = 15 - yOffset;
             
             // Add sway for snow
             let xSway = 0;
             if (weather === WeatherType.Snow) {
                 xSway = Math.sin(time + i) * 0.5;
             }

             dummy.position.set(x + xSway, y, z);
             
             if (weather === WeatherType.Rain) {
                dummy.scale.set(0.05, 0.5, 0.05); // Rain streak
             } else {
                dummy.scale.set(0.1, 0.1, 0.1); // Snowflake
             }
  ...
  return (
      <instancedMesh ref={particlesRef} args={[undefined, undefined, particleCount]} frustumCulled={false}>
          ...
          <meshBasicMaterial 
            color={weather === WeatherType.Rain ? "#a5b4fc" : "#ffffff"} 
            transparent 
            opacity={0.6} 
          />
      </instancedMesh>
...
const EnvironmentEffects = () => {
    return (
        <group raycast={() => null}>
             {/* Clouds - Spaced out for larger map */}
            <Cloud position={[-20, 12, 6]} scale={2.0} speed={0.3} />
            <Cloud position={[8, 14, -12]} scale={1.5} speed={0.5} />
            <Cloud position={[22, 10, 15]} scale={2.2} speed={0.2} />
...
const GroundTile = React.memo(({ type, x, y, grid }: GroundTileProps) => {
  ...
  // Subtle breathing animation for grass tiles
  useFrame((state) => {
    if (type === BuildingType.None && meshRef.current) {
      const time = state.clock.elapsedTime;
      // Calculate a gentle wave offset
      const wave = Math.sin(wx * 0.4 + time * 0.6) * Math.cos(wz * 0.4 + time * 0.4) * 0.03;
      meshRef.current.position.y = centerY + wave;
    }
  });
...
// Zoom Handler Component inside Canvas
const ZoomHandler = ({ zoomApi }: { zoomApi: React.MutableRefObject<{ zoomIn: () => void; zoomOut: () => void; } | null> }) => {
    ...
    zoomApi.current = {
        zoomIn: () => {
            if (camera.type === 'OrthographicCamera') {
                ...
                // Zoom is technically scale-like in ortho, bigger zoom number = closer
                cam.zoom = Math.min(cam.zoom + 10, 100); 
        ...
        zoomOut: () => {
             if (camera.type === 'OrthographicCamera') {
                const cam = camera as THREE.OrthographicCamera;
                cam.zoom = Math.max(cam.zoom - 10, 10);
                cam.updateProjectionMatrix();
            }
        }
    };
...
// Logic to handle Manual Raycasting during Drag
const DragManager = ({ 
    activeDragTool, 
    pointerPosRef, 
    onHoverChange 
    ...
    pointerPosRef: React.MutableRefObject<THREE.Vector2>,
    ...
    useFrame(() => {
        if (!activeDragTool) {
             // Optional: if we want to reset hover when not dragging, we can do it here, 
             // but App.tsx handles clear on drop.
             return; 
        }

        // Raycast using global pointer position
        raycaster.setFromCamera(pointerPosRef.current, camera);
        
        // Intersect with ground tiles
        const intersects = raycaster.intersectObjects(scene.children, true);
        
        // Find first "ground-tile"
        const groundHit = intersects.find(hit => hit.object.name === 'ground-tile');
        
        if (groundHit && groundHit.object.userData) {
            onHoverChange({
                x: groundHit.object.userData.x,
                y: groundHit.object.userData.y
            });
        } else {
            onHoverChange(null);
        }
...
interface IsoMapProps {
  ...
  activeDragTool: BuildingType | null;
  pointerPosRef: React.MutableRefObject<THREE.Vector2>;
  hoveredGridPosRef: React.MutableRefObject<{x: number, y: number} | null>;
  population: number;
  weather: WeatherType;
}

const IsoMap: React.FC<IsoMapProps> = ({ grid, activeDragTool, pointerPosRef, hoveredGridPosRef, population, weather }) => {
  const [hoveredTile, setHoveredTile] = useState<{x: number, y: number} | null>(null);
  const zoomApi = useRef<{ zoomIn: () => void; zoomOut: () => void; } | null>(null);

  // Callback for DragManager to update local state (for preview) and ref (for drop)
  const handleHoverChange = useCallback((tile: {x: number, y: number} | null) => {
    // Only update state if it changed to avoid thrashing
    setHoveredTile(prev => {
        if (prev?.x === tile?.x && prev?.y === tile?.y) return prev;
        return tile;
    });
    hoveredGridPosRef.current = tile;
  }, [hoveredGridPosRef]);

  // Preview Logic
  const showPreview = activeDragTool && hoveredTile && grid[hoveredTile.y][hoveredTile.x].buildingType === BuildingType.None && activeDragTool !== BuildingType.None;
  const previewColor = showPreview ? BUILDINGS[activeDragTool].color : 'white';
  const isBulldoze = activeDragTool === BuildingType.None;
  
  const previewPos = hoveredTile ? gridToWorld(hoveredTile.x, hoveredTile.y) : [0,0,0];

  const lightColor = useMemo(() => {
      if (weather === WeatherType.Rain) return '#a0aec0';
      if (weather === WeatherType.Fog) return '#cbd5e0';
      return '#fffbeb';
  }, [weather]);

  const lightIntensity = useMemo(() => {
      if (weather === WeatherType.Rain || weather === WeatherType.Fog) return 1;
      return 2;
  }, [weather]);
  ...
  return (
    <div className="absolute inset-0 bg-sky-900 touch-none">
      <Canvas shadows dpr={[1, 1.5]} gl={{ antialias: true }}>
        ...
        <MapControls 
          enabled={!activeDragTool} // Disable controls when dragging to avoid sliding
          enableRotate={true}
          enableZoom={true}
          minZoom={15}
          maxZoom={120}
        ...
        <ZoomHandler zoomApi={zoomApi} />
        <DragManager activeDragTool={activeDragTool} pointerPosRef={pointerPosRef} onHoverChange={handleHoverChange} />

        <ambientLight intensity={weather === WeatherType.Rain ? 0.3 : 0.5} color={lightColor} />
        <directionalLight
          castShadow
          position={[15, 20, 10]}
        ...
        {weather === WeatherType.Clear && <Environment preset="city" />}
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-50 pointer-events-auto">
        <button 
          ...
          className="w-12 h-12 bg-white/20 backdrop-blur-md border border-white/50 rounded-full flex items-center justify-center text-white text-2xl font-bold shadow-lg hover:bg-white/30 active:scale-95 transition-all"
          aria-label="Zoom In"
        >
          +
        </button>
